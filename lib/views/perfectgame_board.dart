...
class PerfectGameBoard extends StatefulWidget {
  final bool isHumanVsHuman;

  const PerfectGameBoard({Key? key, required this.isHumanVsHuman})
      : super(key: key);

  @override
  _PerfectGameBoardState createState() => _PerfectGameBoardState();
}

class _PerfectGameBoardState extends State<PerfectGameBoard> {
  ...
  void _botPlay() {
    ...
    // Loop through each row and column to find the first unexplored user ship
    ...
    // If all user ships are sunk, target a random empty cell
    if (targetRow == -1) {
      do {
        targetRow = Random().nextInt(5);
        targetCol = Random().nextInt(5);
      } while (gameBoardMatrix[targetRow][targetCol] != empty);
    }

    // Check if the tile contains the user's ship
    bool isHit = gameBoardMatrix[targetRow][targetCol] == userShip;

    // Update the game board with hit or miss
    gameBoardMatrix[targetRow][targetCol] = isHit ? userHit : aiMiss;
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      setState(() {
        ...
        if (!isHit && gameBoardMatrix[targetRow][targetCol] == aiMiss) {
          gameBoardMatrix[targetRow][targetCol] = empty;
        }
      });
    });
  }
